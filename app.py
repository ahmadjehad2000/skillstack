...
# Custom extension for checkbox handling
class CheckboxExtension(Extension):
    def extendMarkdown(self, md):
        md.preprocessors.register(CheckboxPreprocessor(), 'checkbox', 175)

class CheckboxPreprocessor(markdown.preprocessors.Preprocessor):
    def run(self, lines):
        return [self._convert_checkboxes(line) for line in lines]
    
    def _convert_checkboxes(self, line):
        line = re.sub(r'\[([ ])\]', r'<input type="checkbox" disabled> ', line)
        line = re.sub(r'\[([xX])\]', r'<input type="checkbox" checked disabled> ', line)
        return line

def render_markdown(content):
    """Convert markdown to HTML with proper extensions and security."""
    if not content:
        return ""
    ...
    extensions = [
        ...
        TableExtension(),
        TocExtension(baselevel=1, permalink=True),
        CheckboxExtension()
    ]
...
# Template filter for use in Jinja templates
...
def generate_course():
    ...
    if not api_key:
        flash('Please provide an API key', 'danger')
        return redirect(url_for('index'))
...
def view_module(job_id, module_index):
    """Page to view a generated module with properly rendered markdown content"""
    # Check if job exists and is completed
    if job_id not in job_statuses or job_statuses[job_id]['status'] != 'completed':
        flash('Course not found or still processing', 'warning')
        return redirect(url_for('dashboard'))
    ...
    # Pre-render markdown content for introduction
    if 'introduction' in module_content:
        module_content['introduction_html'] = render_markdown(module_content['introduction'])
    
    # Pre-render markdown content for each lesson
    if 'lessons' in module_content:
        for lesson in module_content['lessons']:
            ...
            if 'assessment' in lesson:
                lesson['assessment_html'] = render_markdown(lesson['assessment'])
